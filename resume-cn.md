...
- 新熊猫直播
    ...
    任职期间完成后端服务架构设计、选型、规划，基于go-micro v3的 **微服务** 架构，服务端开发流程规范，CodeReview等研发向工作。
...
- 2019/10-2020/04：Hiner - 奇安信制品分发系统
    ...
    系统由 **go** 语言实现，以多子服务的方式部署于Docker Swarm集群中，完全通过消息队列耦合。系统对内实现了大文件分片上传、校验、安全扫描（接入天擎、锡安平台。VirusTotal由于成本问题未正式接入），制品包完全递归解压并添加至安全软件的扫描白名单（天擎、安全卫士），CDN推送及自动预热（白山云、金山云、阿里云完整适配），分发权限管理、版本树管理等。对外提供制品索引、版本升级助手等。
...
- 2020/04-2021/01 Fusion - 研发效能数字化度量及流水线平台
    ...
    聚变（fusion）平台是集团 **数字化** 体系中的重要部分，提供研发类数据指标度量及 **Devops** 流水线实现，对研发效能管理提供直接重要的量化支持。
    ...
    平台服务化实现，服务端使用 **go** 语言、**postgresql** 数据库，部署于Swarm集群、前端主要vue实现。
