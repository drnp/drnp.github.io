...
- Infrastructure
    ...
    由于某些原因，于2021年8月停止项目支持，新熊猫直播并未对外发布。
...
### SmartRigger（黑尾科技合作项目）（沈阳龙泽源工业）

智能装配平台 / 工业生产流程数据平台。配合智能电控工具，对铁路系统装配车间的生产状态做出实时反馈，实现数据回溯、统计、分析，工具及工序的管理等，当前用于铁路系统若干机务段、车辆段。
...
- Plus
  - Go-Sicky（自研框架）
  - 完全信创支持（银河麒麟、达梦等支持）
  - 智能量具支持（射频、蓝牙），千分尺，内径尺，游标卡尺等
  - 动态工艺模板
  - 分布式部署方式
  - 定制系统接入及联动
  - 脱网

### Zenkoo直播（黑尾科技技术支持项目）（出海）

完整实现的直播系统，东南亚市场为主。支持多平台、多应用（非马甲包）。采用签约机制（主播与用户数据隔离），可配置的直播流服务。
...
- 服务端
  ...
  - 腾讯云trtc（国际版）
- 移动端
  - 原生Android（kotlin）和iOS（swift）端
  - 腾讯云sdk，可选StreamYard
  - 用户端和主播端分离
- PC端
  - 封装obs-studio
  - 定制DeepFace live，实现实时换脸

**项目当前实施中，预计2025年上半年上架。**
...
当前使用的go脚手架，支持多个进行中的项目。

- 薄封装
- go-fiber http
- uptrace/bun
- spf13/viper
- HTTP / gRPC / websocket / TCP / UDP，server / client
- 多service
- consul / mdns registry

[https://github.com/go-sicky]
...
**项目2023年上架，华为、oppo等市场访问“薇斯姬”，2024年下半年停服**
**官网 <https://wisekin.cn>**
